...
# Anchored ESM-C controlled pair: both arms are 30 layers, so depths are self-matched.
...
PAIR_PRESETS = {
    "ctrl_esmc": dict(
        model_a="ckpt_mlm_s42_token", model_b="ckpt_clm_s42",
        output_root="outputs_ctrl",
        matched_pairs=MATCHED_PAIRS_CTRL_ESMC,
        out_stem="bootstrap_h1_ctrl_esmc",
    ),
    "ctrl": dict(
...
def main():
    ...
    # Overrides: the controlled runs encode seed+protocol in the dir name, so the
    # runner passes them explicitly rather than needing a preset per combination.
    ap.add_argument("--model-a", default=None, help="override preset model_a dir name")
    ap.add_argument("--model-b", default=None, help="override preset model_b dir name")
    ap.add_argument("--out-stem", default=None, help="override preset output stem")
    ap.add_argument("--output-root", default=None,
                    help="override preset output_root (e.g. outputs_ctrl_smoke)")
    ...
    model_a = args.model_a or preset["model_a"]
    model_b = args.model_b or preset["model_b"]
    output_root = ROOT / (args.output_root or preset["output_root"])
    matched_pairs = preset["matched_pairs"]
    out_stem = args.out_stem or preset["out_stem"]
