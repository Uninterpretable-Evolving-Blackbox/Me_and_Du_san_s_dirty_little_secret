...
\item \label{t:blockshuffle} Second destruction procedure: permute contiguous blocks of 16
residues, so short-range order survives and only long-range order is destroyed.
\texttt{bash RUN\_BLOCKSHUFFLE.sh}. About 6 h at three seeds, 3 h at one: the corpus is an hour
and a 42M run is 47 minutes, so this is not the multi-day job it was costed as.
